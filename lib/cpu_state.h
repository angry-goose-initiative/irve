...
/**
 * @brief Holds a RISC-V hart's "CPU state" including registers, the PC, the current load
 * reservation, but not CSRs
*/
class cpu_state_t {
public:
    /**
     * @brief Construct a new cpu_state_t
     * @param CSR_ref A reference to the CSR object that this cpu_state_t object will use
     * internally
     *
    */
...
private:
    ...
    */
     ...
     * 
     * The array size is 31 since we don't bother storing x0. Thus, m_regs[0] is x1 and so on.
    */
    reg_t m_regs[31];

    /**
     * @brief Reference to the CSRs
    */
    CSR::CSR_t& m_CSR_ref;
